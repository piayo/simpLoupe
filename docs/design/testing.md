...
（Vite 本来の `apply: "build"` で指定。`mode` を見て判定すると
`vitest --mode production` のように実行されたときに壊れます）。
あれはビルド時の最適化であって振る舞いではないので、テストは圧縮前のソースの意味を検証します。
