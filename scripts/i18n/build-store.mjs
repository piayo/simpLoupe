 ...
 * 並び順の配列 (g-calize の order) を別に持たないのは、二重管理でズレるため。
...
export function readLocaleDirs( outDir ) {
    try {
        return readdirSync( outDir, { withFileTypes: true } ).filter( d => d.isDirectory() ).map( d => d.name ).toSorted();
    } catch {
        return [];   // 出力先がまだ無い
    }
}
...
function main( argv ) {
    ...
    if ( check ) {
        ...
        console.log( ng
            ? "[build-store] NG: 生成結果と src/_locales/ が一致しません (npm run i18n:build を実行してください)"
            : "[build-store] OK: 生成結果と src/_locales/ は一致しています" );
        return ng ? 1 : 0;
    }
