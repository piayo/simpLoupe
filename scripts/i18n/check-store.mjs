...
import {
    LOCALES_DIR, ROOT_DIR, buildAll, keyNames, loadSource, readExisting, readLocaleDirs,
...
export function checkSource( source ) {
    ...
    for ( const [ locale, label ] of Object.entries( locales ) ) {
        typeof label === "string" && label.trim() !== ""
            || problems.push( error( "locales.json", `${locale}: 値 (言語名のラベル) が文字列でない` ) );
        /^[a-z]{2,3}(_[A-Za-z0-9]+)?$/.test( locale )
            || problems.push( error( "locales.json", `Chrome の locale 形式に見えない: ${locale} (BCP 47 の "-" は UI 側の書式)` ) );
    }
    for ( const key of Object.keys( messages ) ) {
        defined.includes( key )
            || problems.push( error( "messages.json", `keys.json に定義の無いキー: ${key}` ) );
    }
    ...
    for ( const key of keyNames( keys ) ) {
        ...
        for ( const locale of Object.keys( block ) ) {
            localeCodes.includes( locale )
                || problems.push( error( where, `locales.json に無いロケール: ${locale}` ) );
        }
...
export function checkRefs( { source, refs } ) {
    ...
    for ( const { key, where } of refs ) {
        defined.includes( key )
            || problems.push( error( where, `keys.json に定義の無いキーを参照しています: ${key}` ) );
    }
    const used = new Set( refs.map( r => r.key ) );
    for ( const key of defined ) {
        used.has( key )
            || problems.push( error( "keys.json", `どこからも参照されていないキー: ${key} (manifest.json / chrome.i18n.getMessage)` ) );
    }
    return problems;
}
...
export function checkOutput( { built, onDisk, extraDirs, defaultLocale } ) {
    ...
    built.has( defaultLocale )
        || problems.push( error( "manifest.json", `default_locale "${defaultLocale}" が locales.json に無い` ) );
    onDisk.get( defaultLocale )
        || problems.push( error( "src/_locales", `default_locale "${defaultLocale}" のディレクトリが無い` ) );
    ...
    for ( const [ locale, expected ] of built ) {
        ...
        isDeepStrictEqual( current, expected )
            || problems.push( error( "src/_locales", `${locale}: 単一ソースから生成した内容と一致しない (npm run i18n:build)` ) );
    }
    return problems;
}
...
export function collectProblems( { source, manifest, manifestText, files, onDisk, extraDirs } ) {
    const problems = [
        ...checkSource( source ),
        ...checkTranslations( source ),
        ...checkRefs({ source, refs: [ ...extractMsgRefs( manifestText ), ...extractGetMessageRefs( files ) ] }),
    ];
