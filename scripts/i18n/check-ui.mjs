...
export function checkTable(source, listed, fallback) {
    ...
    source.langs.includes(source.fallbackLang) ||
        issues.push({
            level: "error",
            where: "src/i18n/ui",
...
export function scanCalls(text) {
    ...
    for (const m of text.matchAll(CALL_RE)) {
        ...
        lit ? found.push({ key: JSON.parse(`"${lit[1]}"`), line }) : found.push({ expression: arg, line });
    }
    return found;
}
