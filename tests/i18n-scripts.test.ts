...
describe( "check-store", () => {
    ...
    it( "参照されていないキーと未定義キーの参照を検出する", () => {
        const source = storeFixture();
        const refs = [ { key: "extDes", where: "src/manifest.json" }, { key: "nope", where: "src/ts/x.ts:1" } ];
    ...
    it( "manifest.json の __MSG_xxx__ を拾う", () => {
        expect( store.extractMsgRefs( `{ "a": "__MSG_extDes__", "b": "__MSG_extDes__" }` ) )
            .toEqual( [ { key: "extDes", where: "src/manifest.json" } ] );
    });
} );
