...
function send<T>(command: string, data?: unknown, sender?: unknown): Promise<T> {
    return sender === undefined ? sendMessage<T>(mock, command, data) : sendMessage<T>(mock, command, data, sender);
}
...
describe("タブ切り替えでの有効／無効", () => {
    ...
    it.each(["https://example.com/", "http://example.com/page", "https://www.google.com/search?q=a"])("ふつうのページ (%s) では有効にする", async (url) => {
        await activate(url);
    ...
    it.each(["https://chromewebstore.google.com/detail/xxx", "https://chrome.google.com/webstore/detail/xxx", "chrome://extensions", "file:///Users/foo/index.html"])(
        "拡張機能が動けないページ (%s) では無効にする",
        async (url) => {
            await activate(url);
